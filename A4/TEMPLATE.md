...
Fully descibed in section 1. Storage.

# 5. Everything Else

## Main program initialization and termination
After the bytecode for the program is generated, it is going to be loaded directly into the machine memory using the `writeMemory` function for each word. We then set the `MLP` and `MSP` pointers to their correct values.

All programs contain the following initialization code:
```
PUSHMT  # this is going to be base address of main scope activation record.
SETD 0  # display[0] = MSP pointer
PUSH 2  # we just left first two words empty in case we need it in future.
DUPN

# rest of the program follows from here
```
