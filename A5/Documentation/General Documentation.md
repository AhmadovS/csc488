...
* Code generation begins in Main.java's main method/driver, after the full AST tree has been generated and semantic analysis has been completed. As was mentioned before, semantic analysis also calculates the lexical level of each node in the AST tree, since lexical levels are needed in code generation. 
* Main.java starts code generation by calling the doCodeGen method belonging to the AST tree's root node (i.e the main scope of the program). The doCodeGen method that gets called then generates code for initializing the main scope's activation record, right before the doCodeGen method for the root's children get called. This process, whereby a node generates some instructions before calling all of its children's doCodeGen method results in the program's instructions being generated through a depth first traversal of the AST tree. Note that a depth first traversal of the AST tree is done because it allows the code for all of a node A's descendants to be outputted *before* the doCodeGen method for node A has completed. This allows node A to update all of its descendant's branch addresses after the code A and A's descendants have been generated, allowing addresses for operations such as exit statements (that sometimes requires information about a parent node that hasn't been fully processed) to be safely patched at the end of A's doCodeGen method. 
* The instructions generated through this process are written to the machine's memory with the help of the MachineWriter class. After all of the instructions for a program has been generated, Main.java will then call MachineWriter's finishedWriting method to set the machine's registers (e.g program counter, MSP, MLP) to the current values.
* Finally, if all of the preceding steps have been completed, Main.java will call the Machine's run method to begin program execution.
...
Following A4, we have also decided to discontinue using 0 and 1 for booleans, and to instead use MACHINE_FALSE and MACHINE_TRUE as well as NOT and EQ to compare boolean values. 
* This made the code generation phase much less dependent on the way in which the MACHINE class handles boolean values.
* Comparison operators are no longer evaluated using double expressions. Instead, expressions like <= are computed using NOT and/or < operations.

## Testing
* A large number of unit tests were written to ensure that our code generator was free of any potential bugs. A list of tests can be found in tests\index.txt. Each individual test is short and were designed to be run at least once in order to thoroughly test individual parts of our code generation. In order to check to see if a specific test passes, the tester would usually have to look at the data that is expected to reside memory *after* execution, with the data that actually appears in memory after the test has been run.
...
high address -->
				-----------------
				  local vars       (ADDR 0 2 ..)
				-----------------
				  RegB (UNDEFINED) (ADDR 0 1)
				-----------------
				  RegA (UNDEFINED) (ADDR 0 0)
				-----------------
low address  -->
```
...
## Overview
The classes involved in function/procedures are `RoutineDecl`, `RoutineBody`, `IdentExpn`, `ProcedureCallStmt`, `FunctionCallExpn` and `ReturnStmt`.

- **RoutineDecl -** The code generated by this node first adds instructions to skip over the routine body at runtime (we don't want to run the code for the routine body), and it then calls `RoutineBody` to generate the code for the body of the routine.
- **RoutineBody -** This node generates the code for the body of the routine, starting with the display update algorithm, and then generating instructions for the user code. Instructions for procedure exit code are added if no return statement is found in the body of the routine if it is a procedure.
- **IdentExpn, ProcedureCallStmt, FunctionCallExpn -** The code for all these nodes is refactored into `MachineWriter` class `emitCodeRoutineCall` method.
They all start by creaing an activation record, branching to the routine body, and updating display after routine returns.
...
## Checking code paths
Inside the `RoutineBody` node `semanticCheckCodePathsReturnStmt` method, we check all possible code paths (very conservatively), and issue warnings if there's a chance that a code path may not contain a return statement.
...
* We also issue warnings for unreachable statements.

## Calling a routine and creating activation record
Codes for function and procedure calls are produced from the IdentExpn (for function calls without arguments), ProcedureCallStmt, and FunctionCallExpn nodes. There's only slight variation between the code that would be generated by these nodes, all of this code has been refactored into the MachineWriter `emitCodeRoutineCall` method.
...
Note that initially we push `UNDEFINED` value for the return address until the return address is calculated. We want to return to the instruction after the `BR` instruction of the caller that branched to the routine call. After the `BR` instruction is written to memory, we know the address the callee needs to return to, we can then replace the `UNDEFINED` value for the return address.
...
All functions need to have a `return with` statement in all possible code paths, however for procedures, if we think it does not have a `return` statement for all possible code paths, we will add the instructions for exit code automatically.
